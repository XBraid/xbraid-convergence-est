...
outpath=/media/hessenthal/2tb-ext4/data/mgrit-convergence-data
...
do
    ...
    do
        ...
        do
            outname_dteta=range_$rmin\_$rmax\_$imin\_$imax\_num_$rnum\_$inum
            outname=$outname_dteta\_ml_$ml\_$relax\_cf0_$cf0\_bound_$bound\_level_$bound\_rk_$rk
            echo "$outname"
            mpirun -np $np $exe --number-of-timesteps $Nl --number-of-time-grids $ml --coarsening-factors $cf0 \
                                ...
                                --output-file $outname
            sleep 1
            mv dteta_l0.txt dteta_l0_$outname_dteta.txt
            sleep 1
            echo " ==== rendering results"
            python3 utils/heatmap.py dteta_l0_$outname_dteta.txt $outname.txt NAN NAN NAN NAN 0 1
            sleep 1
